...
var parseRestApiServiceError = (response) => __async(void 0, null, function* () {
  if (!response) {
    return;
  }
  const parsedAwsError = yield parseJsonError(stubErrorResponse(response));
  if (!parsedAwsError) ;
  else {
    const bodyText = yield response.body?.text();
    ...
    get(target, prop, receiver) {
      if (prop === "json") {
        return () => __async(this, null, function* () {
          ...
          try {
            return JSON.parse(yield bodyTextPromise);
          } catch (error) {
            return {};
          }
        });
  ...
  const job = () => __async(this, null, function* () {
    try {
      ...
      if (response.statusCode >= 300) {
        throw yield parseRestApiServiceError(response);
      }
      return response;
    } catch (error) {
      if (error.name === "AbortError" || abortSignal?.aborted === true) {
        ...
        if (isTimeout) {
          ...
          const canceledError = new CanceledError(__spreadProps(__spreadValues({}, message && { message }), {
            underlyingError: error,
...
var resolveApiUrl = (amplify, apiName, path, queryParams) => {
  ...
  try {
    ...
    throw new RestApiError(__spreadProps(__spreadValues({
...
var transferHandler = (amplify, options, iamAuthApplicable, signingServiceInfo) => __async(void 0, null, function* () {
  ...
  if (defaultAuthMode !== "none") {
    credentials = yield resolveCredentials(amplify);
  }
  let response;
  ...
  if (isIamAuthApplicable && credentials) {
    ...
    response = yield authenticatedHandler(request, __spreadProps(__spreadValues({}, baseOptions), {
      credentials,
      region: signingRegion,
      service: signingService
    }));
  } else {
    response = yield unauthenticatedHandler(request, __spreadValues({}, baseOptions));
  }
  return {
    statusCode: response.statusCode,
    headers: response.headers,
...
var resolveCredentials = (amplify) => __async(void 0, null, function* () {
  try {
    const { credentials } = yield amplify.Auth.fetchAuthSession();
    if (credentials) {
      return credentials;
    }
  } catch (e) {
...
var publicHandler = (amplify, options, method) => {
  ...
  return createCancellableOperation(
    () => __async(void 0, null, function* () {
      ...
      const libraryConfigHeaders = yield amplify.libraryOptions?.API?.REST?.headers?.({
        apiName
      });
      const { headers: invocationHeaders = {} } = apiOptions;
      const headers = __spreadValues(__spreadValues({}, libraryConfigHeaders), invocationHeaders);
      ...
      return transferHandler(amplify, __spreadProps(__spreadValues({}, apiOptions), {
        url,
        method,
        headers,
        abortSignal
...
var post3 = (amplify, { url, options, abortController }) => {
  ...
  const responsePromise = createCancellableOperation(() => __async(void 0, null, function* () {
    const response = transferHandler(amplify, __spreadProps(__spreadValues({
      url,
      method: "POST"
    }, options), {
...
var ConnectionStateMonitor = class {
  ...
  record(statusUpdates) {
    ...
    const newSocketStatus = __spreadValues(__spreadValues({}, this._linkedConnectionState), statusUpdates);
    this._linkedConnectionState = __spreadValues({}, newSocketStatus);
...
var extractNonAuthHeaders = (headers) => {
  ...
  if ("Authorization" in headers) {
    const _a = headers, { Authorization: _ } = _a, nonAuthHeaders = __objRest(_a, ["Authorization"]);
...
var additionalHeadersFromOptions = (options) => __async(void 0, null, function* () {
  const { appSyncGraphqlEndpoint, query, libraryConfigHeaders = () => ({}), additionalHeaders = {}, authToken } = options;
  let additionalCustomHeaders = {};
  const _libraryConfigHeaders = yield libraryConfigHeaders();
  if (typeof additionalHeaders === "function") {
    const requestOptions = {
      url: appSyncGraphqlEndpoint || "",
      queryString: query || ""
    };
    additionalCustomHeaders = yield additionalHeaders(requestOptions);
  } else {
    additionalCustomHeaders = additionalHeaders;
  }
  if (authToken) {
    additionalCustomHeaders = __spreadProps(__spreadValues({}, additionalCustomHeaders), {
...
var awsAuthTokenHeader = (_0) => __async(void 0, [_0], function* ({ host }) {
...
var awsRealTimeApiKeyHeader = (_0) => __async(void 0, [_0], function* ({ apiKey, host }) {
...
var awsRealTimeIAMHeader = (_0) => __async(void 0, [_0], function* ({ payload, canonicalUri, appSyncGraphqlEndpoint, region }) {
  const endpointInfo = {
    region,
    service: "appsync"
  };
  const creds = (yield fetchAuthSession()).credentials;
  const request = {
    url: `${appSyncGraphqlEndpoint}${canonicalUri}`,
    data: payload,
    method: "POST",
    headers: __spreadValues({}, AWS_APPSYNC_REALTIME_HEADERS)
  };
...
var customAuthHeader = (_0) => __async(void 0, [_0], function* ({ host, additionalCustomHeaders }) {
...
var awsRealTimeHeaderBasedAuth = (_0) => __async(void 0, [_0], function* ({ apiKey, authenticationType, canonicalUri, appSyncGraphqlEndpoint, region, additionalCustomHeaders, payload }) {
  const headerHandler = {
...
var AWSWebSocketProvider = class {
  constructor(args) {
    ...
    this._establishConnection = (awsRealTimeUrl, subprotocol) => __async(this, null, function* () {
      this.logger.debug(`Establishing WebSocket connection to ${awsRealTimeUrl}`);
      try {
        yield this._openConnection(awsRealTimeUrl, subprotocol);
        yield this._initiateHandshake();
      } catch (err) {
  ...
  subscribe(options, customUserAgentDetails) {
    return new Observable((observer) => {
      if (!options?.appSyncGraphqlEndpoint) {
        observer.error({
          errors: [
            __spreadValues({}, new GraphQLError(`Subscribe only available for AWS AppSync endpoint`))
          ]
        });
        observer.complete();
        return;
      ...
      startSubscription();
      return () => __async(this, null, function* () {
        yield this._cleanupSubscription(subscriptionId, reconnectSubscription);
      });
    });
  }
  connect(options) {
    return __async(this, null, function* () {
      ...
      yield this._connectWebSocket(options);
    });
  }
  publish(options, customUserAgentDetails) {
    return __async(this, null, function* () {
      if (this.socketStatus !== SOCKET_STATUS.READY) {
  ...
  _connectWebSocket(options) {
    return __async(this, null, function* () {
      const { apiKey, appSyncGraphqlEndpoint, authenticationType, region } = options;
      const { additionalCustomHeaders } = yield additionalHeadersFromOptions(options);
      this.connectionStateMonitor.record(CONNECTION_CHANGE.OPENING_CONNECTION);
      yield this._initializeWebSocketConnection({
        apiKey,
        appSyncGraphqlEndpoint,
        authenticationType,
        region,
  ...
  _publishMessage(options, customUserAgentDetails) {
    return __async(this, null, function* () {
      const subscriptionId = amplifyUuid();
      const { additionalCustomHeaders, libraryConfigHeaders } = yield additionalHeadersFromOptions(options);
      const serializedSubscriptionMessage = yield this._prepareSubscriptionPayload({
        options,
        subscriptionId,
        customUserAgentDetails,
        additionalCustomHeaders,
  ...
  _cleanupSubscription(subscriptionId, reconnectSubscription) {
    return __async(this, null, function* () {
      reconnectSubscription?.unsubscribe();
      try {
        yield this._waitForSubscriptionToBeConnected(subscriptionId);
  ...
  _startSubscriptionWithAWSAppSyncRealTime(_0) {
    return __async(this, arguments, function* ({ options, observer, subscriptionId, customUserAgentDetails }) {
  ...
  _logStartSubscriptionError(subscriptionId, observer, err) {
    ...
    if (this.connectionState !== ConnectionState.ConnectionDisruptedPendingNetwork) {
      if (isNonRetryableError(err)) {
        observer.error({
          errors: [
            __spreadValues({}, new GraphQLError(`${CONTROL_MSG.CONNECTION_FAILED}: ${message}`))
          ]
        });
      } else {
  ...
  _waitForSubscriptionToBeConnected(subscriptionId) {
    return __async(this, null, function* () {
  ...
  _handleIncomingSubscriptionMessage(message) {
    ...
    if (type === MESSAGE_TYPES.GQL_ERROR || type === MESSAGE_TYPES.EVENT_SUBSCRIBE_ERROR) {
      ...
      if (observer) {
        ...
        observer.error({
          errors: [
            __spreadValues({}, new GraphQLError(`${CONTROL_MSG.CONNECTION_FAILED}: ${errorMessage}`))
          ]
        });
        if (startAckTimeoutId)
          clearTimeout(startAckTimeoutId);
  ...
  _initializeWebSocketConnection({ appSyncGraphqlEndpoint, authenticationType, apiKey, region, additionalCustomHeaders }) {
    ...
    return new Promise((resolve4, reject) => __async(this, null, function* () {
      this.promiseArray.push({ res: resolve4, rej: reject });
      if (this.socketStatus === SOCKET_STATUS.CLOSED) {
        try {
          this.socketStatus = SOCKET_STATUS.CONNECTING;
          const payloadString = "{}";
          const authHeader = yield awsRealTimeHeaderBasedAuth({
            authenticationType,
          ...
          yield this._establishRetryableConnection(awsRealTimeUrl, authTokenSubprotocol);
  ...
  _establishRetryableConnection(awsRealTimeUrl, subprotocol) {
    return __async(this, null, function* () {
      this.logger.debug(`Establishing retryable connection`);
      yield jitteredExponentialRetry(this._establishConnection.bind(this), [awsRealTimeUrl, subprotocol], MAX_DELAY_MS);
    });
  }
  _openConnection(awsRealTimeUrl, subprotocol) {
    return __async(this, null, function* () {
      return new Promise((resolve4, reject) => {
...
var AWSAppSyncRealTimeProvider = class extends AWSWebSocketProvider {
  ...
  _prepareSubscriptionPayload(_0) {
    return __async(this, arguments, function* ({ options, subscriptionId, customUserAgentDetails, additionalCustomHeaders, libraryConfigHeaders }) {
      ...
      const headers = __spreadProps(__spreadValues(__spreadValues(__spreadValues({}, yield awsRealTimeHeaderBasedAuth({
        apiKey,
        appSyncGraphqlEndpoint,
        authenticationType,
      ...
      const subscriptionMessage = {
        ...
        payload: {
          data: serializedData,
          extensions: {
            authorization: __spreadValues({}, headers)
          }
        },
...
function headerBasedAuth(_0, _1, _2) {
  return __async(this, arguments, function* (amplify, authMode, apiKey, additionalHeaders = {}) {
    let headers = {};
    switch (authMode) {
      case "apiKey":
        if (!apiKey) {
        ...
        break;
      case "iam": {
        const session = yield amplify.Auth.fetchAuthSession();
      ...
      case "userPool": {
        let token;
        try {
          token = (yield amplify.Auth.fetchAuthSession()).tokens?.accessToken.toString();
        } catch (e) {
          throw new GraphQLApiError(__spreadProps(__spreadValues({}, NO_SIGNED_IN_USER), {
            underlyingError: e
          }));
        }
        if (!token) {
...
var InternalGraphQLAPIClass = class {
    ...
    switch (operationType) {
      ...
      case "mutation": {
        ...
        if (isAmplifyInstance(amplify)) {
          ...
          const wrapper = (amplifyInstance) => __async(this, null, function* () {
            const result = yield this._graphql(amplifyInstance, { query, variables, authMode, apiKey, endpoint }, headers, abortController, customUserAgentDetails, authToken);
            return result;
          });
  ...
  _graphql(_0, _1) {
    return __async(this, arguments, function* (amplify, { query, variables, authMode: authModeOverride, endpoint: endpointOverride, apiKey: apiKeyOverride }, additionalHeaders = {}, abortController, customUserAgentDetails, authToken) {
      ...
      const authHeaders = yield headerBasedAuth(amplify, authMode, apiKeyOverride ?? apiKey, additionalCustomHeaders);
      const headers = __spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues({}, !customEndpoint && authHeaders), customEndpoint && (customEndpointRegion ? authHeaders : {}) || {}), customHeaders && (yield customHeaders({
        query: print(query),
        variables
      ...
      let response;
      try {
        const { body: responseBody } = yield this._api.post(amplify, {
        ...
        response = yield responseBody.json();
      } catch (error) {
...
function initializeModel(client, modelName, result, modelIntrospection, authMode, authToken, context = false) {
  ...
  return result.map((record) => {
          ...
          if (context) {
            initializedRelationshipFields[fieldName] = (contextSpec, options) => {
              if (record[targetNames[0]]) {
                return client.models[relatedModelName].get(contextSpec, __spreadValues({
            ...
            initializedRelationshipFields[fieldName] = (options) => {
              if (record[targetNames[0]]) {
                return client.models[relatedModelName].get(__spreadValues({
                  [relatedModelPKFieldName]: record[targetNames[0]]
                }, sortKeyValues), {
                  authMode: options?.authMode || authMode,
                  authToken: options?.authToken || authToken
        ...
        case connectionType.HAS_MANY: {
          ...
          if (relatedModelFieldType.model) {
            ...
            if (context) {
              initializedRelationshipFields[fieldName] = (contextSpec, options) => {
                if (record[parentPk]) {
                  return selfAwareAsync((resultPromise) => __async(this, null, function* () {
                    ...
                    return mapResult(yield extendedBase);
                  }));
                }
                return [];
              };
    ...
    return __spreadValues(__spreadValues({}, record), initializedRelationshipFields);
  });
}
...
function generateGraphQLDocument(modelIntrospection, modelDefinition, modelOperation, listArgs, indexMeta) {
  ...
  switch (modelOperation) {
    ...
    case "LIST":
      graphQLArguments ?? (graphQLArguments = __spreadProps(__spreadValues({}, listPkArgs), {
    ...
    case "INDEX_QUERY":
      graphQLArguments ?? (graphQLArguments = __spreadProps(__spreadValues({}, indexQueryArgs), {
        filter: `Model${namePascalCase}FilterInput`,
        sortDirection: "ModelSortDirection",
        limit: "Int",
        nextToken: "String"
...
function _op(client, modelIntrospection, operationType, operation, getInternals2, args, options, context, customUserAgentDetails) {
  ...
  `;
    ...
    try {
      const basePromise = context ? client.graphql(context, __spreadProps(__spreadValues({}, auth), {
        query,
        variables
      }), headers) : client.graphql(__spreadValues(__spreadProps(__spreadValues({}, auth), {
        query,
        variables
      }), userAgentOverride), headers);
      const extendedPromise = extendCancellability(basePromise, resultPromise);
      const { data, extensions } = yield extendedPromise;
      if (data) {
...
function _opSubscription(client, modelIntrospection, operation, getInternals2, args, options, customUserAgentDetails) {
  ...
  const variables = operationVariables(operation, args);
  const userAgentOverride = createUserAgentOverride(customUserAgentDetails);
  const observable = client.graphql(__spreadValues(__spreadProps(__spreadValues({}, auth), {
    query,
    variables
  }), userAgentOverride), headers);
...
function _list(client, modelIntrospection, model, getInternals2, args, contextSpec, customUserAgentDetails) {
  return selfAwareAsync((resultPromise) => __async(this, null, function* () {
    ...
    const userAgentOverride = createUserAgentOverride(customUserAgentDetails);
    try {
      const basePromise = contextSpec ? client.graphql(contextSpec, __spreadProps(__spreadValues({}, auth), {
        query,
        variables
      }), headers) : client.graphql(__spreadValues(__spreadProps(__spreadValues({}, auth), {
        query,
...
var deserializeImageBlock = ({ image }) => ({
  image: __spreadProps(__spreadValues({}, image), {
    source: __spreadProps(__spreadValues({}, image.source), {
      bytes: fromBase64(image.source.bytes)
    })
  })
});
var deserializeDocumentBlock = ({ document }) => ({
  document: __spreadProps(__spreadValues({}, document), {
    source: __spreadProps(__spreadValues({}, document.source), {
...
var deserializeToolUseBlock = ({ toolUse }) => ({
  toolUse: __spreadProps(__spreadValues({}, toolUse), {
...
var createListMessagesFunction = (client, modelIntrospection, conversationId, conversationMessageModel, getInternals2) => (input) => __async(void 0, null, function* () {
  ...
  const { data, nextToken, errors } = yield list(__spreadProps(__spreadValues({}, input), {
...
var deserializeToolUseBlock2 = (contentBlockToolUse) => {
  if (contentBlockToolUse) {
    const toolUseBlock = __spreadProps(__spreadValues({}, contentBlockToolUse), {
...
var serializeToolResultBlock = ({ toolResult }) => ({
  toolResult: __spreadProps(__spreadValues({}, toolResult), {
    content: toolResult.content.map((toolResultBlock) => {
      if (toolResultBlock.image) {
...
var createSendMessageFunction = (client, modelIntrospection, conversationId, conversationRouteName, getInternals2) => (input) => __async(void 0, null, function* () {
  const { conversations } = modelIntrospection;
  if (!conversations) {
    return {};
  }
  ...
  const { data, errors } = yield sendOperation(__spreadValues(__spreadValues({
    conversationId,
    content: serializeContent(content)
  }, aiContext && { aiContext: serializeAiContext(aiContext) }), toolConfiguration && {
...
var createCreateConversationFunction = (client, modelIntrospection, conversationRouteName, conversationModel, conversationMessageModel, getInternals2) => (input) => __async(void 0, null, function* () {
  ...
  const { data, errors } = yield createOperation({ name, metadata });
  return {
...
var createGetConversationFunction = (client, modelIntrospection, conversationRouteName, conversationModel, conversationMessageModel, getInternals2) => (_0) => __async(void 0, [_0], function* ({ id }) {
  ...
  const { data, errors } = yield get5({ id });
  return {
...
var createListConversationsFunction = (client, modelIntrospection, conversationRouteName, conversationModel, conversationMessageModel, getInternals2) => (input) => __async(void 0, null, function* () {
  ...
  const { data, nextToken, errors } = yield list(input);
  return {
    data: data.map((datum) => {
...
var createDeleteConversationFunction = (client, modelIntrospection, conversationRouteName, conversationModel, conversationMessageModel, getInternals2) => (_0) => __async(void 0, [_0], function* ({ id }) {
  ...
  const { data, errors } = yield deleteOperation({ id });
  return {
    data: data ? convertItemToConversation(client, modelIntrospection, data?.id, data?.createdAt, data?.updatedAt, conversationRouteName, conversationMessageModel, getInternals2, data?.metadata, data?.name) : data,
    errors
  };
...
var createUpdateConversationFunction = (client, modelIntrospection, conversationRouteName, conversationModel, conversationMessageModel, getInternals2) => (_0) => __async(void 0, [_0], function* ({ id, metadata: metadataObject, name }) {
  const metadata = JSON.stringify(metadataObject);
  ...
  const { data, errors } = yield updateOperation({ id, metadata, name });
  return {
    data: data ? convertItemToConversation(client, modelIntrospection, data?.id, data?.createdAt, data?.updatedAt, conversationRouteName, conversationMessageModel, getInternals2, data?.metadata, data?.name) : data,
    errors
  };
...
function generateConversationsProperty(client, apiGraphQLConfig, getInternals2) {
  ...
  for (const { name, conversation, message, models, nonModels, enums } of Object.values(modelIntrospection.conversations)) {
    ...
    const conversationModelIntrospection = __spreadProps(__spreadValues({}, modelIntrospection), {
      models: __spreadValues(__spreadValues({}, modelIntrospection.models), models),
      nonModels: __spreadValues(__spreadValues({}, modelIntrospection.nonModels), nonModels),
      enums: __spreadValues(__spreadValues({}, modelIntrospection.enums), enums)
    });
    conversations[name] = {
...
function indexQueryFactory(client, modelIntrospection, model, indexMeta, getInternals2, context = false) {
  const indexQueryWithContext = (contextSpec, args, options) => {
    return _indexQuery(client, modelIntrospection, model, indexMeta, getInternals2, __spreadValues(__spreadValues({}, args), options), contextSpec);
  };
  const indexQuery = (args, options) => {
    return _indexQuery(client, modelIntrospection, model, indexMeta, getInternals2, __spreadValues(__spreadValues({}, args), options));
  };
...
function _indexQuery(client, modelIntrospection, model, indexMeta, getInternals2, args, contextSpec) {
  return selfAwareAsync((resultPromise) => __async(this, null, function* () {
    ...
    try {
      const headers = getCustomHeaders(client, getInternals2, args?.headers);
      const graphQlParams = __spreadProps(__spreadValues({}, auth), {
        query,
        variables
      });
      ...
      const extendedPromise = extendCancellability(basePromise, resultPromise);
      const response = yield extendedPromise;
...
function subscriptionFactory(client, modelIntrospection, model, operation, getInternals2) {
  ...
  const subscription = (args) => {
    ...
    const auth = authModeParams(client, getInternals2, args);
    const headers = getCustomHeaders(client, getInternals2, args?.headers);
    const observable = client.graphql(__spreadProps(__spreadValues({}, auth), {
      query,
      variables
    }), headers);
    return observable.pipe(map((value) => {
  ...
  const observeQuery = (arg) => new Observable((subscriber) => {
    ...
    (() => __async(this, null, function* () {
      ...
      while (!subscriber.closed && (firstPage || nextToken)) {
        firstPage = false;
        const { data: page, errors, nextToken: _nextToken } = yield models[name].list(__spreadProps(__spreadValues({}, arg), { nextToken }));
...
var GraphQLAPIClass = class extends InternalGraphQLAPIClass {
  ...
  graphql(amplify, options, additionalHeaders) {
    ...
    if (isGraphQLOptionsWithOverride(options)) {
      const _b = options, { [_a = INTERNAL_USER_AGENT_OVERRIDE]: internalUserAgentOverride } = _b, cleanOptions = __objRest(_b, [__restKey(_a)]);
      return super.graphql(amplify, cleanOptions, additionalHeaders, __spreadValues(__spreadValues({}, userAgentDetails), internalUserAgentOverride));
    }
    return super.graphql(amplify, options, additionalHeaders, __spreadValues({}, userAgentDetails));
  }
  /**
   * Checks to see if an error thrown is from an api request cancellation
...
function generateClient2(options) {
  return generateClient(__spreadProps(__spreadValues({}, options || {}), {
    amplify: Amplify
  }));
}
...
var CONNECT_URI2 = "";
var AWSAppSyncEventProvider = class _AWSAppSyncEventProvider extends AWSWebSocketProvider {
  constructor() {
    super({
  ...
  connect(options) {
    return __async(this, null, function* () {
      return __superGet(_AWSAppSyncEventProvider.prototype, this, "connect").call(this, options);
    });
  }
  subscribe(options, customUserAgentDetails) {
  ...
  publish(options, customUserAgentDetails) {
    return __async(this, null, function* () {
      return __superGet(_AWSAppSyncEventProvider.prototype, this, "publish").call(this, options, customUserAgentDetails);
    });
  }
  ...
  _prepareSubscriptionPayload(_0) {
    return __async(this, arguments, function* ({ options, subscriptionId, customUserAgentDetails, additionalCustomHeaders, libraryConfigHeaders, publish }) {
...
function appsyncRequest(_0, _1) {
  return __async(this, arguments, function* (amplify, options, additionalHeaders = {}, abortController, customUserAgentDetails) {
    ...
    const headers = yield requestHeaders(amplify, options, additionalHeaders, customUserAgentDetails);
    const body = {
      channel: query,
      events: variables
    };
    ...
    const { body: responseBody } = yield post3(amplify, {
      url: new AmplifyUrl(endpoint),
      options: {
        headers,
        body,
    ...
    const response = yield responseBody.json();
    if (isGraphQLResponseWithErrors(response)) {
      throw repackageUnauthorizedError(response);
    }
    return response;
...
function requestHeaders(amplify, options, additionalHeaders, customUserAgentDetails) {
  return __async(this, null, function* () {
    ...
    const authHeaders = yield headerBasedAuth(amplify, authMode, apiKey, additionalCustomHeaders);
    const customHeaders = customHeadersFn && (yield customHeadersFn({
      query,
      variables
    }));
    const headers = __spreadProps(__spreadValues(__spreadValues(__spreadValues({}, authHeaders), customHeaders), additionalCustomHeaders), {
      // User agent headers:
...
function connect(channel, options) {
  return __async(this, null, function* () {
    ...
    yield AppSyncEventProvider.connect(providerOptions);
    ...
    const sub = (observer, subOptions) => {
      ...
      const subscribeOptions = __spreadProps(__spreadValues({}, providerOptions), { query: channel });
    ...
    const pub = (event, pubOptions) => __async(this, null, function* () {
      if (!openChannels.has(channelId)) {
        throw new Error("Channel is closed");
      }
      const publishOptions = __spreadProps(__spreadValues({}, providerOptions), {
        query: channel,
        variables: event
      });
...
function post4(channel, event, options) {
  return __async(this, null, function* () {
    const providerOptions = configure();
    providerOptions.authenticationType = normalizeAuth(options?.authMode, providerOptions.authenticationType);
    providerOptions.apiKey = options?.apiKey || providerOptions.apiKey;
    ...
    const abortController = new AbortController();
    const res = yield appsyncRequest(Amplify, publishOptions, {}, abortController);
...
function closeAll() {
  return __async(this, null, function* () {
    yield AppSyncEventProvider.close();
  });
}
export {
  ApiError,
