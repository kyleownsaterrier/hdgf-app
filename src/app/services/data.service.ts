...
const SAVE_DOUBLES = /* GraphQL */ `
  mutation SaveDoublesTable($id: ID!, $rows: AWSJSON!, $caliPlayer: String) {
...
const SAVE_BAGTAGS = /* GraphQL */ `
  mutation SaveBagTagsTable($id: ID!, $rows: AWSJSON!) {
...
export class DataService {
  ...
  async loadDoubles(): Promise<{ rows: TableRow[]; caliPlayer: string } | null> {
    try {
      ...
      const table = result.data?.getDoublesTable;
  ...
  async loadBagTags(): Promise<BagTagRow[] | null> {
    try {
      ...
      const table = result.data?.getBagTagsTable;
      if (!table) return null;
